...
| 3 | `+` `-` | 加算・減算 |
| 4 | `=` `==` `<>` `!=` `>` `<` `>=` `<=` | 比較 |
| 5 | `&` `^` `\|` | ビット論理積（AND）・排他的論理和（XOR）・論理和（OR） |
| 6 | `&&` | 論理積（AND） |
| 7 | `\|\|` | 論理和（OR） |

`>=` は `≧`、`<=` は `≦`、`<>` と `!=` は `≠` として扱われます。
単一の `&` `^` `|` はビット演算子として数値をビットごとに演算し、`&&` `||` は真偽値判定用の論理演算子として動作します。
`&` `^` `|` は同じ優先順位で、左から順に評価されます。

```mml
Int A = 1 | 2   // A = 3 (ビットOR)
Int B = 3 & 1   // B = 1 (ビットAND)
Int C = 3 ^ 1   // C = 2 (ビットXOR)
IF (A > 10 && B < 5) { cde }
```
...
`NoteNo` の引数には、音符をMMLでそのまま書きます。オクターブ指定(`o5` や `>` `<` `` ` ``)や臨時記号(`+` `-`)、音符番号を指定する `n` 命令も使えます。オクターブを省略した場合は、その時点のトラックのオクターブを使います。オクターブは演奏時と同じく0～10の範囲に丸めます。`Key`(KeyShift)や `TrackKey` の指定も反映するため、実際に鳴る音の番号が返ります。
