    ...
    while !cur.is_eos() {
        ...
        match ch {
            ...
            */
            ...
            'p' => result.push(read_pitch_bend_small(&mut cur, song)), // @ pitch bend - ピッチベンドの指定 range:0-127 (center:64) (ex) p64 / p.onTime(low,high,len) / p.onNoteWave(low,high,len) / p%n で -8192~0~8191 の指定 (ex) p%=0 / (ref) PB(n) は -8192~0~8191
