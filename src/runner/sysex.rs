...
pub(super) fn exec_sysex(song: &mut Song, t: &Token) {
    ...
    if args.len() == 0 {
        runtime_error(
            ...
            &format!(
                "SysEx : {}",
                song.get_message(MessageKind::ErrorWrongArguments)
            ),
        );
        return;
    }
...
pub(super) fn exec_sysex_reset(song: &mut Song, t: &Token) {
    ...
    match t.value_i {
        0 => {
            // GM
            song.add_event(Event::sysex_raw(
                time,
                vec![0xF0, 0x7E, 0x7F, 0x9, 0x1, 0xF7],
            ));
        }
        1 => {
            // GS
            song.add_event(Event::sysex_raw(
                time,
                vec![
                    0xF0, 0x41, dev, 0x42, 0x12, 0x40, 0x00, 0x7F, 0x00, 0x41, 0xF7,
                ],
            ));
        }
        2 => {
            ...
            song.add_event(Event::sysex_raw(
                time,
                vec![0xF0, 0x43, dev, 0x4c, 0x00, 0x00, 0x7e, 0x00, 0xf7],
            ));
        }
        _ => {}
    }
...
pub(super) fn exec_gs_effect(song: &mut Song, t: &Token) {
    ...
    match &t.value_i {
        ...
        0x15 => {
            // change to the rhytm part
            let val = if data.len() >= 1 {
                data[0].to_i() as u8
            } else {
                0
            };
