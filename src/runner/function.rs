...
pub(super) fn exec_userfunc_or_array_or_macro(song: &mut Song, t: &Token) -> bool {
        ...
        match var {
            ...
            SValue::Array(a) => {
                ...
                if args.len() == 0 {
                    runtime_error(
                        song,
                        &format!("get Array({}) element needs arguments", name),
                    );
                    return false;
                }
            ...
            SValue::Str(src, _) => {
                ...
                if args.len() == 0 {
                    runtime_error(
                        song,
                        &format!("get String({}) element needs arguments", name),
                    );
                    return false;
                }
                // replace string
    ...
    if song.functions.len() <= func_id {
        runtime_error(
            song,
            &format!("broken func_id={} in exec_call_user_function", func_id),
        );
        return false;
    }
    ...
    for i in 0..song.functions[func_id].arg_names.len() {
        ...
        let mut v: SValue = if i < args.len() {
            args[i].clone()
        } else {
            SValue::None
        };
...
pub(super) fn exec_sys_function(song: &mut Song, t: &Token) -> bool {
    ...
    match func_val {
        SValue::UserFunc(_func_id) => {
            if exec_userfunc_or_array_or_macro(song, t) {
                return true;
            }
        }
...
pub(super) fn get_system_value(cmd: &str, song: &Song) -> Option<SValue> {
    ...
    if cmd == "TR" || cmd == "TRACK" || cmd == "Track" {
        // @ get current track no - 現在のトラック番号を得る
        let tr = song.cur_track as isize;
        return Some(SValue::from_i(tr));
    }
    if cmd == "CH" || cmd == "CHANNEL" {
        // @ get current channel no - 現在のチャンネル番号を得る
        let ch = trk!(song).channel + 1; // range: 1-16
        return Some(SValue::from_i(ch));
    }
    if cmd == "TIME" || cmd == "Time" || cmd == "TIMEPOS" || cmd == "TIMEPTR" {
        // @ get time posision - 現在のタイムポインタ値を得る
        let v = trk!(song).timepos;
        return Some(SValue::from_i(v));
    }
    if cmd == "TEMPO" || cmd == "Tempo" || cmd == "BPM" {
        // @ get tempo - 現在のテンポ値を得る
        let v = song.tempo;
        return Some(SValue::from_i(v));
    }
    if cmd == "KEY" || cmd == "KEY_SHIFT" {
        // @ get key shift - 現在のキーシフト値を得る
        let v = song.key_shift;
        return Some(SValue::from_i(v));
    }
    if cmd == "TR_KEY" || cmd == "TrackKey" {
        // @ get track key shift - 現在のトラックごとのキーシフト値を得る
        let v = trk!(song).track_key;
        return Some(SValue::from_i(v));
    }
    if cmd == "TIMEBASE" || cmd == "Timebase" {
        // @ get timebase - 現在のタイムベース値を得る
        let v = song.timebase;
        return Some(SValue::from_i(v));
    }
    if cmd == "l" {
        // @ get length - 現在のlの値を得る
        let v = trk!(song).length;
        return Some(SValue::from_i(v));
    }
    if cmd == "v" {
        // @ get velocity - 現在のvの値を得る
        let v = trk!(song).velocity;
        return Some(SValue::from_i(v));
    }
    if cmd == "q" {
        // @ get gate rate - 現在のqの値を得る
        let v = trk!(song).qlen;
        return Some(SValue::from_i(v));
    }
    if cmd == "o" {
        // @ get octave rate - 現在のoの値を得る
...
pub(crate) fn var_extract(val: &SValue, song: &mut Song) -> SValue {
    match val {
        ...
        SValue::Str(s, _) => {
            if s.starts_with('=') && s.len() >= 2 {
                ...
                match song.variables_get(key) {
                    Some(v) => v.clone(),
                    None => match get_system_value(key, song) {
                        Some(v) => return v,
                        None => {
        ...
        SValue::Array(values) => SValue::Array(
            values
                .iter()
                .map(|value| var_extract(value, song))
                .collect(),
        ),
        // Other value
        _ => val.clone(),
