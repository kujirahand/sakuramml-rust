...
| ---------|---------|--------|
| "全音符" | "l1" | 全音符を基本音符にする |
| "二分音符" | "l2" | 二分音符を基本音符にする |
| "四分音符" | "l4" | 四分音符を基本音符にする |
| "八分音符" | "l8" | 八分音符を基本音符にする |
| "十六音符" | "l16" | 十六音符を基本音符にする |
| "付点四分音符" | "l4." | 付点四分音符を基本音符にする |
| "音源初期化" | "System.MeasureShift(1ResetGM;Time(1:1:0TrackSync;" | 音源初期化//音源の初期化(GMリセット)を実行する。（例）音源初期化 |
| "音長" | "l" | 基本音符を指定 |
| "音量予約" | "v.onTime=" | 音量を予約指定する |
| "「" | "'" | 和音はじめ |
| "」" | "'" | 和音おわり |
| "【" | "[" | 繰り返しはじめ |
| "】" | "]" | 繰り返しおわり |
...
| "−" | "-" | マイナス |
| "‘" | "`" | 次の音符をオクターブ1つ上げる |
| "調" | "System.KeyFlag" | 調#(音符)//臨時記号を設定する。（例）調＃（ドファ） |
| "音階" | "o" | 音階(数値)//音階を数値で指定する。初期値は５。範囲は、0～10（例）音階５ |
| "時間" | "Time" | 時間(小節数:拍数:ステップ数)//指定時間にポインタを移動する。範囲は、小節数・拍数が、１～。ステップ数は、０～。（例）時間（４：１：０） |
| "読む" | "Include" | 読む(ファイル名)//外部定義ファイルを読み込む。（例）読む(chord2.h) |
| "予約" | ".onNote=" | (コマンド)予約(v1,v2,v3...)//コマンドの値を予約しておく（例）音量予約120,50【ドレミファ】 |
| "拍子" | "System.TimeSignature=" | 拍子 分子,分母//拍子を設定する。（例）拍子4,4 |
| "音色" | "@" | 音色（番号）//音色を設定する。 |
| "音符" | "l" | 音符（ｎ分音符指定）//基本となる音符の長さをｎ分音符で指定する。（例）音符16//１６分音符の意味 |
| "音量" | "v" | 音量（数値）//音量(実際は音の強さ)を設定する。初期値は、100。範囲は、0~127。（例）音量127 |
| "連符" | "Div" | 連符{音名}[音長]//３連符や５連符などを表現する。（例）連符{ドレミ}4 |
| "ゲート" | "q" | ゲート（割合）//音符の長さに対する実際の発音時間を割合（100分率）で指定する。範囲は、1～100～。（例）ゲート80 |
| "テンポ" | "Tempo=" | テンポ（数値）//テンポを設定する。初期値は、120。範囲は、20～240を推奨。（例）テンポ120 |
| "曖昧さ" | ".Random=" | (コマンド)曖昧さ（数値）//各属性の曖昧さを設定する。範囲は、0～。初期値は、0。（例）音量曖昧さ80 【ドレミソ】 |
| "トラック" | "Track=" | トラック（番号）//トラック番号を指定する。初期値は、０。範囲は、0～。（例）トラック３ |
| "チャンネル" | "Channel=" | チャンネル（番号）//現在のトラックにチャンネルを設定する。初期値は、トラック番号と同じ。範囲は、１～１６（例）トラック３チャンネル１０ |
| "曲名" | "TrackName=" | 曲名{"文字列"}//生成するMIDIファイルに曲名を埋め込む。（例）曲名{"テスト"} |
| "作者" | "Copyright=" | 作者{"文字列"}//生成するMIDIファイルに著作権情報を埋め込む。（例）作者{"クジラ飛行机"} |
| "コメント" | "MetaText=" | コメント{"文字列"}//生成するMIDIファイルにコメントを埋め込む。（例）コメント{"テスト"} |
| "演奏位置" | "PlayFrom" | 演奏位置(小節数:拍数:ステップ数))//長い曲の途中から演奏したい時、曲の演奏位置を指定する。（例）演奏位置（32:1:0） |
...
| "ろ" | "n43 | Highタム |
| "く" | "n44 | ドラム |
| "大きく" | "Cresc=" | 大きく(音長),(最終値)//音量(EP)をだんだん大きくする |
| "小さく" | "Decresc=" | 小さく(音長),(最終値)//音量(EP)をだんだん小さくする |
| "クレッシェンド" | "Cresc=" | 大きく(音長),(最終値)//音量(EP)をだんだん大きくする |
| "デクレッシェンド" | "Cresc=" | 小さく(音長),(最終値)//音量(EP)をだんだん小さくする |
| "音量戻す" | "EP(127)" | 音量(EP)を最大値に戻す |
| "方向左" | "P(0)" | ステレオの左から音が出るようにする |
| "方向左前" | "P(32)" | ステレオの左前から音が出るようにする |
| "方向前" | "P(64)" | ステレオの前から音が出るようにする |
| "方向右前" | "P(96)" | ステレオの右前から音が出るようにする |
| "方向右" | "P(127)" | ステレオの右から音が出るようにする |
| "方向回す" | "P.onNoteWaveEx(0 | ステレオの左右を回す |
| "ビブラートオフ" | "M(0)" | ビブラートをやめる |
| "ペダル" | "y64 | ペダルを踏む |
| "放す" | "y64 | ペダルを放す |
| "テンポ改" | "TempoChange=" | テンポ改([[[t1],t2],len])//テンポを推移的に変更する。lenを省略すると、全音符の間に推移し、t1を省略すると、以前の値からt2へ推移する。 |
| "ビブラート" | "M.onNoteWaveEx(0 | 推移的なビブラートをかける |
| "ここから演奏" | "PlayFrom(Time" | 途中から演奏したいときに書く |